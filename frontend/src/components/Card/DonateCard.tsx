...
export default function DonateCard({ title, price, oldPrice, image }: DonateCardProps) {

    const perksList = [
        "Баннихоп",
        "Больше денег в начале раунда",
        "Выделение в чате и табе",
        "Резервный слот",
        "Медшприц в начале раунда",
        "105 HP + 105 ARMOR",
    ];

    return (
        <div className="w-full max-w-95 bg-[#161616]/80 backdrop-blur-sm rounded-2xl border border-white/10 overflow-hidden
        hover:shadow-blue-900/20 transition-all duration-500 group">
            ...
            <div className="mt-6 px-6 flex items-center justify-center gap-3 relative z-10">
                {oldPrice && (
