...
import { IconBrandSteam, IconCreditCard } from "@tabler/icons-react";
...
export default function Header() {
    ...
    return (
        <header className="w-full max-w-330 mx-auto px-8 pt-6 pb-2 flex items-center justify-between">
                ...
                <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2">
                        ...
                        <button className="flex items-center gap-2.5 px-5 py-2.5 rounded-xl border border-white/10 bg-blue-500/80 text-gray-300 text-sm font-semibold cursor-pointer
                            ...
                            <IconCreditCard className="w-5 h-5 text-white/60 group-hover:text-white transition-colors" />
                            <span>Пополнить</span>
                        </button>
                    </div>
                    <DropdownMenu />
            ...
            <AnimatePresence>
                {isOpenModal && <PaymentModal onClose={() => setIsOpenModal(false)}/>}
            </AnimatePresence>
